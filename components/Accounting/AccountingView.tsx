...
import HalfModal from '../Common/HalfModal';
...
const AccountingView: React.FC = () => {
  ...
  const openAddModal = () => {
    ...
    setInDate(format(new Date(), 'yyyy-MM-dd'));

    // 시간 자동 설정 (현재 시간에서 가장 가까운 10분 단위)
    const now = new Date();
    setInHour(now.getHours());
    setInMin(Math.floor(now.getMinutes() / 10) * 10);
  ...
  const getModalTitle = () => {
    if (workMode === '추가') return '새로운 내역 추가';
    if (workMode === '수정') return '내역 수정';
    if (workMode === '복사') return '내역 복사 추가';
    return '내역 입력';
  };
    ...
    if (workMode === '수정' && editingEntryId) {
      setSheets(sheets.map(s => s.id === activeSheetId ? {
        ...s, entries: s.entries.map(e => e.id === editingEntryId ? {
          ...e, date: inDate, hour: inHour, minute: inMin, type: inType as '수입' | '지출', item: inItem,
      ...
      const newEntry = {
        ...
        type: inType as '수입' | '지출', item: inItem, incomeAmount: inType === '수입' ? amt : 0,
    ...
    reader.onload = (evt) => {
      try {
        ...
        const imported = data.map(row => {
          // 1. 시간 데이터 분해 복구 (파일 내 '시' 또는 '시간' 대응)
          const timeStr = String(row['시'] || row['시간'] || '09:00');
          const [h, m] = timeStr.split(':').map(v => parseInt(v) || 0);

          // 2. 항목 데이터 필드 매핑 복구 (파일 내 '항목' 또는 '내역' 대응)
          const itemValue = String(row['항목'] || row['내역'] || row['수입/지출 내역'] || '');

          const incomeValue = Number(row['수입금액'] || row['수입'] || 0);
          const expenseValue = Number(row['지출금액'] || row['지출'] || 0);
          ...
          return {
            ...
            hour: h,
            minute: m,
            type: (incomeValue > 0 ? '수입' : '지출') as '수입' | '지출',
            item: itemValue,
            incomeAmount: incomeValue,
            expenseAmount: expenseValue,
            balance: 0
          };
        }).filter(e => e.date !== '합계'); // 합계 행 제외
  ...
  return (
    <div className="flex flex-col h-full bg-[#0a0a0a] text-gray-200 overflow-hidden font-sans select-none">
      ...
      <div className="flex items-center bg-[#000] border-b border-[#1a1a2e] px-4 h-14 shrink-0">
        <div className="flex-grow"><h2 className="text-xl md:text-2xl font-black text-white tracking-tighter">회계장부</h2></div>
