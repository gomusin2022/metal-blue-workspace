...
const MemberView: React.FC<MemberViewProps> = ({ members, setMembers, onHome }) => {
  ...
  const handleDbDownload = (e: React.MouseEvent) => {
    ...
    const dbData = targetMembers.map(m => ({
      id: m.sn.toString(), name: m.name, position: m.position, phone: m.phone, branch: m.branch, join_year: m.joined, addr: m.address, fee: m.fee ? "1" : "", car_num: m.carNumber, attendance: m.attendance ? (m.attendance === true ? "1" : m.attendance) : "", note: m.memo
    }));
    ...
    <div className="flex flex-col h-full bg-[#121212] p-1.5 md:p-6 pt-0.5 text-gray-200 overflow-hidden font-sans">
      <div className="flex flex-col w-full mb-1">
        <div className="flex items-center justify-between w-full h-10">
          {isEditingTitle ? (
            <input autoFocus className="text-[1.35rem] md:text-[1.8rem] font-black text-white bg-transparent border-b border-blue-500 outline-none w-1/2" value={memberTitle} onChange={(e) => setMemberTitle(e.target.value)} onBlur={() => setIsEditingTitle(false)} onKeyDown={(e) => { if(e.key === 'Enter') setIsEditingTitle(false); }} />
          ) : (
            <h2 className="text-[1.35rem] md:text-[1.8rem] font-black text-white truncate cursor-pointer hover:text-blue-400" onClick={() => setIsEditingTitle(true)} title="클릭하여 제목 수정">{memberTitle}</h2>
          )}
          <div className="flex bg-[#1a1a2e] p-1 rounded border border-[#3a3a5e] gap-1.5 shadow-lg shrink-0">
            <button title="문자전송" onClick={handleMessageSend} className="p-1.5 text-orange-400 hover:bg-orange-500/10 rounded"><MessageSquare className="w-5 h-5" /></button>
            <button title="선택삭제" onClick={() => { if(selectedIds.size === 0) return alert("삭제할 대상을 선택하세요."); if(confirm(`${selectedIds.size}명을 삭제할까요?`)) { setMembers(members.filter(m => !selectedIds.has(m.id))); setSelectedIds(new Set()); } }} className="p-1.5 text-red-500 hover:bg-red-500/10 rounded"><Eraser className="w-5 h-5" /></button>
            <button title="회원추가" onClick={() => { setEditingMember({ id: generateId(), sn: 0, branch: '본점', name: '', position: '회원', phone: '010--', address: '', joined: '', fee: false, attendance: false, carNumber: lastSelectedCar, memo: '' }); setIsModalOpen(true); }} className="p-1.5 text-blue-500 hover:bg-blue-500/10 rounded"><UserPlus className="w-5 h-5" /></button>
            ...
            <label title="디비업" className="p-1.5 text-indigo-500 cursor-pointer hover:bg-indigo-500/10 rounded"><CloudUpload className="w-5 h-5" /><input type="file" className="hidden" accept=".db,.json" onChange={handleDbUpload} /></label>
          </div>
        </div>
        <div className="flex items-center justify-between w-full border-t border-[#3a3a5e]/20 pt-1.5">
          <div className="flex gap-1 overflow-x-auto no-scrollbar pr-2">
            {[ {label:'지점', key:'branch'}, {label:'이름', key:'name'}, {label:'차량', key:'carNumber'}, {label:'회비', key:'fee'}, {label:'출결', key:'attendance'}, {label:'가입', key:'joined'} ].map(btn => (
              <button key={btn.key} onClick={() => setSortCriteria(prev => prev.includes(btn.key) ? prev.filter(x => x !== btn.key) : [btn.key, ...prev])} className={`px-2 py-1 min-w-[44px] rounded border text-[10px] md:text-xs font-black transition-all ${sortCriteria.includes(btn.key) ? 'bg-blue-600 border-blue-400 text-white' : 'bg-[#1a1a2e] border-[#3a3a5e] text-gray-400'}`}>{btn.label}</button>
            ))}
          </div>
          <div className="flex items-center gap-1.5 shrink-0 ml-auto font-black text-[10px] text-gray-300">
            <select className="bg-[#1a1a2e] border border-blue-500/50 rounded px-1.5 py-1 text-blue-400 outline-none appearance-none pr-4" value={selectedBranch} onChange={(e) => setSelectedBranch(e.target.value)}>{branches.map(b => <option key={b} value={b} className="bg-[#121212]">{b}</option>)}</select>
      ...
      {isModalOpen && editingMember && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-md p-4 animate-in fade-in duration-300">
          <div className="w-full max-w-lg bg-[#1a1a2e] rounded-[2rem] p-8 shadow-[0_20px_50px_rgba(0,0,0,0.5)] border border-white/10 relative overflow-hidden">
            <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-transparent via-blue-500 to-transparent opacity-50" />
            <div className="flex items-center justify-between mb-8"><h3 className="text-xl font-black text-white tracking-tight">{editingMember.id && members.find(m => m.id === editingMember.id) ? '정보 수정' : '새 회원 등록'}</h3><button onClick={() => setIsModalOpen(false)} className="p-2 hover:bg-white/10 rounded-full transition-colors text-gray-400 hover:text-white"><X className="w-5 h-5" /></button></div>
            <div className="space-y-6">
              <div className="flex gap-4">
                <div className="flex-1 space-y-1.5"><label className="text-[10px] text-blue-400 font-black ml-1 uppercase tracking-widest">Branch</label><select className="w-full bg-white/5 border border-white/10 rounded-2xl px-4 py-3.5 text-white font-bold outline-none focus:border-blue-500/50 focus:bg-blue-500/5 transition-all appearance-none cursor-pointer" value={editingMember.branch} onChange={(e) => setEditingMember({...editingMember, branch: e.target.value})}>{branches.filter(b => b !== '전체').map(b => <option key={b} value={b} className="bg-[#1a1a2e]">{b}</option>)}</select></div>
                <div className="flex-[2] space-y-1.5"><label className="text-[10px] text-blue-400 font-black ml-1 uppercase tracking-widest">Address</label><input className="w-full bg-white/5 border border-white/10 rounded-2xl px-4 py-3.5 text-white font-bold outline-none focus:border-blue-500/50 focus:bg-blue-500/5 transition-all placeholder:text-white/20" value={editingMember.address} onChange={(e) => setEditingMember({...editingMember, address: e.target.value})} placeholder="상세 주소 입력" /></div>
              </div>
              <div className="flex gap-4">
                <div className="flex-1 space-y-1.5"><label className="text-[10px] text-blue-400 font-black ml-1 uppercase tracking-widest">Name</label><input className="w-full bg-white/5 border border-white/10 rounded-2xl px-4 py-3.5 text-white font-bold outline-none focus:border-blue-500/50 focus:bg-blue-500/5 transition-all placeholder:text-white/20" value={editingMember.name} onChange={(e) => setEditingMember({...editingMember, name: e.target.value})} placeholder="성함" /></div>
                <div className="flex-[2] space-y-1.5"><label className="text-[10px] text-blue-400 font-black ml-1 uppercase tracking-widest">Phone</label><div className="flex items-center gap-2 bg-white/5 border border-white/10 rounded-2xl px-4 py-3.5 text-white focus-within:border-blue-500/50 focus-within:bg-blue-500/5 transition-all"><span className="font-black text-gray-500">010</span><span className="text-white/20">-</span><input ref={phoneMidRef} type="tel" className="w-full bg-transparent font-black text-center outline-none" value={(editingMember.phone || '').split('-')[1] || ''} onChange={(e) => { const v = e.target.value.replace(/\D/g,'').slice(0,4); const p = (editingMember.phone || '010--').split('-'); setEditingMember({...editingMember, phone: `010-${v}-${p[2]||''}`}); if(v.length===4) phoneEndRef.current?.focus(); }} maxLength={4} /><span className="text-white/20">-</span><input ref={phoneEndRef} type="tel" className="w-full bg-transparent font-black text-center outline-none" value={(editingMember.phone || '').split('-')[2] || ''} onChange={(e) => { const v = e.target.value.replace(/\D/g,'').slice(0,4); const p = (editingMember.phone || '010--').split('-'); setEditingMember({...editingMember, phone: `010-${p[1]||''}-${v}`}); }} maxLength={4} /></div></div>
              </div>
              <div className="flex gap-4 pt-6"><button onClick={() => setIsModalOpen(false)} className="flex-1 py-4 bg-white/5 hover:bg-white/10 text-white rounded-[1.25rem] font-black transition-all border border-white/5 active:scale-95">취소</button><button onClick={handleModalSave} className="flex-[2] py-4 bg-blue-600 hover:bg-blue-500 text-white rounded-[1.25rem] font-black flex items-center justify-center gap-2 shadow-[0_10px_25px_rgba(37,99,235,0.4)] transition-all active:scale-95"><Save className="w-5 h-5" />저장하기</button></div>
            </div>
          </div>
        </div>
      )}
